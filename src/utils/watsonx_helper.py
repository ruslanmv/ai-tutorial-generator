#!/usr/bin/env python3
"""
src/utils/watsonx_helper.py

Fetch active (non-deprecated, non-withdrawn) Watsonx foundation models
that are NOT embeddings from multiple regions, then display:

 1. Active non-embedding model counts per region
 2. Which non-embedding models are missing (per region) compared to US-South
 3. Which non-embedding models are unique to each region
 4. Non-embedding models common to all regions
 5. A master list of all non-embedding models and the regions (short codes) in which each appears
"""
...
# Exclude embedding-capable models and withdrawn/deprecated ones
PARAMS = {
    "version": "2024-09-16",
...
def fetch_active_non_embedding_models() -> Dict[str, Set[str]]:
    """Fetch and return a mapping of region → active non-embedding model_ids."""
    ...
    for base in BASE_URLS:
        ...
        try:
            ...
            logging.info(" → %d active non-embedding models", len(active))
...
def print_region_summary(model_sets: Dict[str, Set[str]]):
    ...
    print("\nActive Non-Embedding Models per Region:")
...
def print_common_models(common: List[str]):
    print("\nNon-Embedding Models Present in ALL Regions:")
    if common:
        for m in common:
            print(f"  • {m}")
    else:
...
def print_model_regions(model_sets: Dict[str, Set[str]]):
    """
    Print a table listing every non-embedding model and the short region codes
    in which it appears.
    """
    ...
    print("\nAll Non-Embedding Models and Their Regions (short codes):")
...
def main():
    logging.info("Starting non-embedding comparison tool")
    model_sets = fetch_active_non_embedding_models()
    ...
    # 5) Master list of all non-embedding models and their regions (short codes)
